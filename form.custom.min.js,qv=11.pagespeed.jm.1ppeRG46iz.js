function sendFormDataTmp(field, value, target) {
    var data = {}, form = target.closest("form");
    data.form_update_from_script = 1;
    data.request_id = tmp_data_request_id;
    data.form_data = {};
    switch (field) {
        case"name":
            if (value.length > 4) {
    ...
    if (isJsonEnable) {
        try {
            var dataRow = form.serializeArray(), addressIsset = false,
              fieldsDisabled = ['s1', 's2', 's3', 'offer_id', 'landing', 'campaign', 'order_source', 'request_id', 'return_url', 'timezone', 'jswork', 'frompopup', 'address', 'client_data'];
            $(dataRow).each(function (item, itemData) {
                if (itemData.name == "address") {
                    addressIsset = true;
                }
                if (fieldsDisabled.includes(itemData.name)) {
                    delete dataRow[item];
                }
            });
            if (!addressIsset) {
...
$(document).ready(function () {
    $("form").sisyphus({excludeFields: $("[name=country], [name=language], [name=s1], [name=s2], [name=s3], [name=offer_id], [name=landing], [name=campaign], [name=request_id], [name=referer], [name=order_source], [name=return_url], [name=timezone], [name=jswork], [name=frompopup]"),});
    ...
    $("form input, form select").not("input[name='name'], input[name='phone'], select[name='country']").blur(function () {
        var target = $(this);
        if (target.val() != "") {
            sendFormDataTmp(target.attr("name"), target.val(), target);
        }
